...
class AbstractBinMetric(AbstractMetric):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        self._bin_weights = ut.compute_bin_weights(bin_indices=self._bin_indices, sample_weight=self._masked_weight)
        return self


class BinBasedSDE(AbstractBinMetric):
    def __init__(self, uniform_features, uniform_label, n_bins=10, target_rcp=None, power=2.0):
        """
        ...
        """
        AbstractBinMetric.__init__(self, n_bins=n_bins, uniform_features=uniform_features, uniform_label=uniform_label)
        self.power = power
    ...
    def __call__(self, y, proba, sample_weight):
        ...
        for cut in cuts:
            bin_efficiencies = ut.compute_bin_efficiencies(
                y_pred, bin_indices=self._bin_indices, cut=cut, sample_weight=self._masked_weight
            )
            result += ut.weighted_deviation(bin_efficiencies, weights=self._bin_weights, power=self.power)

        return (result / len(cuts)) ** (1.0 / self.power)
...
class BinBasedCvM(AbstractBinMetric):
    def __init__(self, uniform_features, uniform_label, n_bins=10, power=2.0):
        """
    ...
    def __call__(self, y, proba, sample_weight):
        ...
        for bin, bin_weight in enumerate(self._bin_weights):
            ...
            result += bin_weight * ut._cvm_2samp_fast(
                global_data, local_distribution, global_weight, local_weights, global_cdf
            )
        return result
...
class KnnBasedSDE(AbstractKnnMetric):
    def __init__(self, uniform_features, uniform_label, n_neighbours=50, target_rcp=None, power=2.0):
        """
        ...
        :param float power: power used in SDE formula (default is 2.)
        """
        AbstractKnnMetric.__init__(
            self, n_neighbours=n_neighbours, uniform_features=uniform_features, uniform_label=uniform_label
        )
        self.power = power
        self.target_rcp = target_rcp
    ...
    def __call__(self, y, proba, sample_weight):
        ...
        for cut in cuts:
            groups_efficiencies = ut.compute_group_efficiencies(
                y_pred, groups_matrix=self._group_matrix, cut=cut, divided_weight=self._divided_weights
            )
...
class KnnBasedCvM(AbstractKnnMetric):
    def __init__(self, uniform_features, uniform_label, n_neighbours=50, power=2.0):
        """
    ...
    def __call__(self, y, proba, sample_weight):
        ...
        for group, group_weight in zip(self._groups_indices, self._group_weights):
            ...
            result += group_weight * ut._cvm_2samp_fast(
                global_data, local_distribution, global_sample_weight, local_sample_weights, global_cdf
            )
        return result
